...
namespace MineFields
{
    public class MineField : IMineField
    {
        public string[,] GetHintField(MFSings[,] mineField)
        {
            ...
            var rows = FieldHelper<MFSings>.GetRowsNumber(mineField);
            var columns = FieldHelper<MFSings>.GetColumnsNumber(mineField);
            ...
            for (var i = 0; i < rows; i++)
            {
                ...
                {
                    if (mineField[i, j] == MFSings.Star)
                    {
                        hintField[i, j] = -1;

                        if (i - 1 >= 0 && j - 1 >= 0 && mineField[i - 1, j - 1] != MFSings.Star)
                        {
                            hintField[i - 1, j - 1] += 1;
                        }

                        if (i - 1 >= 0 && j >= 0 && mineField[i - 1, j] != MFSings.Star)
                        {
                            hintField[i - 1, j] += 1;
                        }
                        ...
                        if (i >= 0 && j - 1 >= 0 && mineField[i, j - 1] != MFSings.Star)
                        {
                            hintField[i, j - 1] += 1;
                        }
